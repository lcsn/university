...
public class GpioPwmService {
	...
	public Response startPwm(@PathParam("pin") Integer pin) {
		Response response;
		try {
			response = gpioHelper.newGpioPwmPin(pin);
	...
	public Response startPwm(@PathParam("pin") Integer pin, @PathParam("power") Integer power, @PathParam("frequency") Integer frequency) throws GpioException {
		return gpioHelper.newGpioPwmPin(pin, power, frequency);
	}
	...
	@PUT
	@Path("/adjust/{pin}/frequency/{frequency}")
	public Response frequency(@PathParam("pin") Integer pin, @PathParam("frequency") Integer frequency) throws Exception {
		return GpioPwmOutputPinController.getInstance().adjustFrequency(gpioHelper.toRaspiPin(pin), frequency);
	}
